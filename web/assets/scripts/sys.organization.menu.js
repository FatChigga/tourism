iv = {
    ...
    bind: function () {
        var roleSetting = {
            ...
            callback: {
                onClick: function (event, treeId, treeNode) {
                    ...
                    $.get(basePath + '/sys/organizationMenu/menuList', {
                        'id': treeNode.id
                    }, function (data) {
        ...
        $.get(basePath + '/sys/organizationMenu/organizationMenuNodes', null, function (data) {
            zTreeObj = $.fn.zTree.init($("#tree-organization"), roleSetting, data[0]);
            zTreeObj = $.fn.zTree.init($("#tree-menu"), menuSetting, data[1]);
        })
    },
    edit: function () {
        var menuTree = $.fn.zTree.getZTreeObj("tree-menu");
        var organiztionTree = $.fn.zTree.getZTreeObj("tree-organization");
        var nodes = organiztionTree.getSelectedNodes();

        if (nodes.length == 0 || nodes[0].id == 0) {
            $.mdlg.alert('提示', '请选组织机构');
            return;
        }
        ...
        if (typeof(nodes[0]) != 'undefined') {
            params['organizationId'] = nodes[0].id;
        }
        ...
        $.post(basePath + 'sys/organizationMenu/save', JSON.stringify(params), function (data) {
            if (data.result == true) {
                var treeObj = $.fn.zTree.getZTreeObj("tree-menu");
                var checkedNodes = treeObj.getCheckedNodes();
                $("#btn-save").data("checkedNodes", checkedNodes);
